...
in conjunction with the format guidelines below, to ensure that your TREP submission won’t be rejected because of form.
...
replacing all this sample text with your own text, and be sure to adhere to the format guidelines below. 
- Try to adhere to the pattern (at most one) sentence a line[^2] (also[^3]) to make it easier to review TREPs in Pull Requests.
...
- Once the TREP is getting ready to be merged, the TR core team member determines the next free TREP number, assigns it to the current TREP (typically in a comment in the pull request).
