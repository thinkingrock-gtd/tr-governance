...
- Once the TREP is getting ready to be merged, the TR core team sponsor communicates, typically in a comment in the pull request, three data: the number to be assigned to the current TREP (typically the next free TREP number), the new TREP status (in general "Active"), and the discussion period (usually from one week to three months).
